...
import BHomeSearch from '@/modules/home/components/BHomeSearch.vue'
import BWaterfall from '@/modules/home/components/BWaterfall.vue'
import BCategory from '@/modules/home/components/BCategory.vue'
import { onMounted, ref } from 'vue'
import { pageHomePicturesUsingPost } from '@/services/api/homeController.ts'
...
const fetchHomePictureList = async () => {
  ...
  try {
    ...
    failCount.value++
    console.error(`加载图片失败 (第${failCount.value}次):`, error)
    if (failCount.value >= MAX_FAIL_COUNT) {
      hasMoreData.value = false
      console.warn('请求连续失败，已停止自动加载')
    }
  } finally {
