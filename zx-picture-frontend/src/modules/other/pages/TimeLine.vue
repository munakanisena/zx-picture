<template>
  ...
  <div style="max-width: 800px; margin: 0 auto; padding: 2rem 1.5rem">
    ...
    <n-timeline size="large">
      ...
      >
        <n-card :title="item.date" embedded hoverable style="border-radius: 8px; max-width: 800px">
          <template #header-extra>
            <n-tag :type="getStateColor(item.state)" round>{{ item.state }}</n-tag>
          </template>
...
const timeLineItemList: timeLineItem[] = [
  {
    date: '2026-4-13',
    title: '优化项目结构',
    content: ['用户模块优化', '前端页面优化', '团队成员添加优化', '空间数据分析优化', '添加限流逻辑','图片分类'],
    state: '已完成'
  },
  {
    date: '2025-07-19',
