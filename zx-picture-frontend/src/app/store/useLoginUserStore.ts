...
import { getLoginUserDetailUsingGet, userLogoutUsingPost } from '@/services/api/userController.ts'
  ...
  () => {
    ...
    async function refreshUser() {
      ...
      try {
        ...
        const bizCode = e?.data?.code
        if (bizCode === 40100) {
          userInfo.value = {}
          return
        }
        console.warn('刷新用户信息失败，保留当前状态:', bizCode ?? e)
      }
    }

    return { userInfo, isLogin, updateAvatar, setLoginUser, logout, refreshUser, checkLogin }
  },
  //存储选项
  {
    persist: {
