 ...
 * AI扩图图片校验
 */
export const validateImageForExtend = (picture: API.PictureDetailVO) => {
  const allowedFormats = ['jpg', 'jpeg', 'png', 'heif', 'webp']
  const format = picture.originFormat?.toLowerCase()
  if (format && !allowedFormats.includes(format)) {
    return { valid: false, message: `AI扩图仅支持 JPG、JPEG、PNG、HEIF、WEBP 格式，当前格式：${picture.originFormat}` }
  }
  if (picture.originSize && picture.originSize > 10 * 1024 * 1024) {
    return {
      valid: false,
      message: `AI扩图要求图片大小不超过10MB，当前大小：${(picture.originSize / (1024 * 1024)).toFixed(1)}MB`,
    }
  }
  const w = picture.originWidth
  const h = picture.originHeight
  if (w && (w < 512 || w > 4096)) {
    return { valid: false, message: `AI扩图要求图片宽度在512~4096像素之间，当前宽度：${w}px` }
  }
  if (h && (h < 512 || h > 4096)) {
    return { valid: false, message: `AI扩图要求图片高度在512~4096像素之间，当前高度：${h}px` }
  }
