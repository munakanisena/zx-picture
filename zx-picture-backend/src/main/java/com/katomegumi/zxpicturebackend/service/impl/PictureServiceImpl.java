...
public class PictureServiceImpl extends ServiceImpl<PictureInfoMapper, PictureInfo> implements PictureService {
    ...
    public LambdaQueryWrapper<PictureInfo> getLambdaQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        lambdaQueryWrapper
                ...
                .and(StrUtil.isNotBlank(searchText), lqw -> lqw.and(wrapper -> wrapper.like(PictureInfo::getPicName, searchText).or().like(PictureInfo::getPicDesc, searchText).or().apply("FIND_IN_SET ('" + searchText + "', tags) > 0"))).eq(ObjUtil.isNotNull(id), PictureInfo::getId, id).eq(StrUtil.isNotBlank(originFormat), PictureInfo::getOriginFormat, originFormat).eq(ObjUtil.isNotNull(originWidth), PictureInfo::getOriginWidth, originWidth).eq(ObjUtil.isNotNull(originHeight), PictureInfo::getOriginHeight, originHeight).eq(ObjUtil.isNotNull(originScale), PictureInfo::getOriginScale, originScale).eq(StrUtil.isNotBlank(originColor), PictureInfo::getOriginColor, originColor).like(StrUtil.isNotBlank(picName), PictureInfo::getPicName, picName).like(StrUtil.isNotBlank(picDesc), PictureInfo::getPicDesc, picDesc).eq(ObjUtil.isNotNull(categoryId), PictureInfo::getCategoryId, categoryId).eq(ObjUtil.isNotNull(userId), PictureInfo::getUserId, userId).eq(ObjUtil.isNotNull(spaceId), PictureInfo::getSpaceId, spaceId).eq(ObjUtil.isNotNull(reviewerId), PictureInfo::getReviewerId, reviewerId).eq(ObjUtil.isNotNull(reviewStatus), PictureInfo::getReviewStatus, reviewStatus).like(StrUtil.isNotBlank(reviewMessage), PictureInfo::getReviewMessage, reviewMessage);
