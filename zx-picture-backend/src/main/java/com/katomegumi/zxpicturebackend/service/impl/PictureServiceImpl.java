...
public class PictureServiceImpl extends ServiceImpl<PictureInfoMapper, PictureInfo> implements PictureService {
    ...
    public PageVO<PictureHomeVO> getCollectPictureList(PictureQueryRequest pictureQueryRequest) {
        ...
        return new PageVO<>(pictureInfoPage.getCurrent(), pictureInfoPage.getSize(), pictureInfoPage.getTotal(), pictureInfoPage.getPages(), Optional.ofNullable(pictureInfoPage.getRecords()).orElse(Collections.emptyList()).stream().map(pictureInfo -> {
    ...
    public UserPictureStatsVO getUserPictureStats() {
        ...
        Long collectCount = pictureInteractionService.lambdaQuery().eq(PictureInteraction::getUserId, userId).eq(PictureInteraction::getInteractionType, PictureInteractionTypeEnum.COLLECTION.getKey()).eq(PictureInteraction::getInteractionStatus, PictureInteractionStatusEnum.INTERACTED.getKey()).count();
    ...
    public PageVO<PictureVO> getPicturePageListAsPersonSpace(PictureQueryRequest pictureQueryRequest) {
        ...
        Page<PictureInfo> pictureInfoPage = this.page(pictureQueryRequest.getPage(PictureInfo.class), lambdaQueryWrapper);
        PageVO<PictureVO> pictureVOPageVO = new PageVO<>(pictureInfoPage.getCurrent(), pictureInfoPage.getSize(), pictureInfoPage.getTotal(), pictureInfoPage.getPages(), Optional.ofNullable(pictureInfoPage.getRecords()).orElse(Collections.emptyList()).stream().map(pictureInfo -> BeanUtil.copyProperties(pictureInfo, PictureVO.class)).collect(Collectors.toList()));
    ...
    public PageVO<PictureVO> searchPictureByPicColor(SearchPictureByColorRequest searchPictureByColorRequest) {
        ...
        //查看空间全部图(要有主色调的图)
        Page<PictureInfo> page = this.lambdaQuery().eq(PictureInfo::getSpaceId, spaceId).isNotNull(PictureInfo::getOriginColor).page(searchPictureByColorRequest.getPage(PictureInfo.class));

        //将picColor(16进制) 转换RGB
        Color color = Color.decode(picColor);

        return new PageVO<>(page.getCurrent(), page.getSize(), page.getTotal(), page.getPages(), Optional.ofNullable(page.getRecords()).orElse(Collections.emptyList()).stream().sorted(Comparator.comparingDouble((PictureInfo pictureInfo) -> {
        ...
        }).reversed()).map(PictureVO::objToVo).collect(Collectors.toList()));
    }
    ...
    public PageVO<PictureVO> getPicturePageListAsManage(PictureQueryRequest pictureQueryRequest) {
        ...
        PageVO<PictureVO> pictureVOPageVO = new PageVO<>(page.getCurrent(), page.getSize(), page.getTotal(), page.getPages(), Optional.ofNullable(page.getRecords()).orElse(Collections.emptyList()).stream().map(PictureVO::objToVo).collect(Collectors.toList()));
    ...
    public LambdaQueryWrapper<PictureInfo> getLambdaQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        lambdaQueryWrapper
                ...
                .and(StrUtil.isNotBlank(searchText), lqw -> lqw.and(wrapper -> wrapper.like(PictureInfo::getPicName, searchText).or().like(PictureInfo::getPicDesc, searchText).or().apply("FIND_IN_SET({0}, tags) > 0", searchText))).eq(ObjUtil.isNotNull(id), PictureInfo::getId, id).eq(StrUtil.isNotBlank(originFormat), PictureInfo::getOriginFormat, originFormat).eq(ObjUtil.isNotNull(originWidth), PictureInfo::getOriginWidth, originWidth).eq(ObjUtil.isNotNull(originHeight), PictureInfo::getOriginHeight, originHeight).eq(ObjUtil.isNotNull(originScale), PictureInfo::getOriginScale, originScale).eq(StrUtil.isNotBlank(originColor), PictureInfo::getOriginColor, originColor).like(StrUtil.isNotBlank(picName), PictureInfo::getPicName, picName).like(StrUtil.isNotBlank(picDesc), PictureInfo::getPicDesc, picDesc).eq(ObjUtil.isNotNull(categoryId), PictureInfo::getCategoryId, categoryId).eq(ObjUtil.isNotNull(userId), PictureInfo::getUserId, userId).eq(ObjUtil.isNotNull(spaceId), PictureInfo::getSpaceId, spaceId).eq(ObjUtil.isNotNull(reviewerId), PictureInfo::getReviewerId, reviewerId).eq(ObjUtil.isNotNull(reviewStatus), PictureInfo::getReviewStatus, reviewStatus).like(StrUtil.isNotBlank(reviewMessage), PictureInfo::getReviewMessage, reviewMessage);
