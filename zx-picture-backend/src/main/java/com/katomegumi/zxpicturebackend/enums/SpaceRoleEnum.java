...
public enum SpaceRoleEnum {
    ...
    VIEWER("viewer", "浏览者"), EDITOR("editor", "编辑者"), ADMIN("admin", "管理员");
    ...
    public static List<String> getLabels() {
        return Arrays.stream(values()).map(SpaceRoleEnum::getLabel).collect(Collectors.toList());
    }
    ...
    public static List<String> getKeys() {
        return Arrays.stream(values()).map(SpaceRoleEnum::getKey).collect(Collectors.toList());
    }
}
