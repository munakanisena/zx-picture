...
public class PictureController {
    ...
    public BaseResponse<Boolean> likeOrCollection(@RequestBody PictureInteractionRequest pictureInteractionRequest) {
        ...
        ThrowUtils.throwIf(ObjUtil.hasNull(pictureInteractionRequest.getInteractionStatus(), pictureInteractionRequest.getInteractionType()), ErrorCode.PARAMS_ERROR);
        //根据枚举 校验类型规范
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> deletePictureByAdmin(@RequestBody DeleteRequest deleteRequest) {
        ThrowUtils.throwIf(deleteRequest == null, ErrorCode.PARAMS_ERROR);
        Long pictureId = deleteRequest.getId();
        ThrowUtils.throwIf(pictureId == null || pictureId <= 0, ErrorCode.PARAMS_ERROR);
        pictureService.deletePictureById(pictureId);
