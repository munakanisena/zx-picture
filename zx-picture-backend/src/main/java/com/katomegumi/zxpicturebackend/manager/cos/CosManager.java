...
public class CosManager {
    ...
    public String getImageAve(String key) {
        GetObjectRequest getObj = new GetObjectRequest(cosClientConfig.getBucketName(), key);
        // 获取图片主色调
        ...
        try (COSObjectInputStream objectContent = object.getObjectContent()) {
            //获取流中json字符串
            String jsonStr = IoUtil.read(objectContent, StandardCharsets.UTF_8);
            //解析并获取RGB
            return JSONUtil.parseObj(jsonStr).getStr("RGB");
        } catch (Exception e) {
            log.error("获取图片主色调失败,消息:{}", e.getMessage(), e);
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "获取图片主色调失败");
        }
    }
}
